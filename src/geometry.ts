...
class Point {
	...
	distanceTo(obj: Point  | Rect) {
		if (obj instanceof Point) return vec.dist(this.coord, obj.coord)
			...
			if (this.coord[1] < obj.a[1] || this.coord[1] > obj.b[1]) distA = Math.hypot(vertDistA, vertDistB)
			else inside++
			
			if (this.coord[0] < obj.a[0] || this.coord[0] > obj.b[0]) distB = Math.hypot(vertDistA, vertDistB)
			else inside++
			
			const modul = Math.min(distA, distB)
			
			if (inside == 2) return -modul
			return modul
		}
	}
}
...
class Segment {
	...
	distanceTo(obj: Point): number {
		return obj.distanceTo(this) as number
	}
}
...
class Rect {
	...
	constructor(coord: number[], size: number[]) {
		size = vec.mult(size, .5)
		this.a = vec.sub(coord, size)
		this.b = vec.add(coord, size)
	}
	distanceTo(obj: Point): number {
		return obj.distanceTo(this)as number
	}
...
export const geometry = {
	point: (coord: number[]) => new Point(coord),
	segment: (a: number[], b: number[]) => new Segment(a, b),
	rect: (coord: number[], size: number[]) => new Rect(coord, size),
